...
use rspack_error::{error, miette::{miette, LabeledSpan, SourceOffset}, Result};
...
pub fn serde_error_to_miette(e: serde_json::Error, content: Arc<str>, msg: &str) -> rspack_error::miette::Report{
   let offset = SourceOffset::from_location(content.as_ref(),e.line(), e.column());
          ...
          miette!(
            labels = vec![span],
            "{msg}").with_source_code(content.clone())
}
pub fn get_builtin_loader(builtin: &str, options: Option<&str>) -> Result<BoxLoader> {
  ...
  if builtin.starts_with(SWC_LOADER_IDENTIFIER) {
    return Ok(Arc::new(
      rspack_loader_swc::SwcLoader::new(serde_json::from_str(options.as_ref()).map_err(
        |e| {
  ...
  if builtin.starts_with(LIGHTNINGCSS_LOADER_IDENTIFIER) {
    let config: rspack_loader_lightningcss::config::RawConfig =
      serde_json::from_str(options.as_ref()).map_err(|e| {
        serde_error_to_miette(e, options, "Could not parse builtin:lightningcss-loader options")
    })?;
